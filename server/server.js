...
const app = express();
// Порт, на котором сервер будет слушать запросы. Railway (и большинство
// облачных хостингов) сам назначает порт и передаёт его через
// process.env.PORT — сервер обязан слушать именно его, а не то, что
// захочется нам. Локально этой переменной нет, поэтому остаётся 3000 —
// тот же порт, что раньше использовал json-server, чтобы фронтенд
// ничего не заметил.
...
//
// Локально фронт и сервер сидят на одном localhost, но после деплоя
// фронт будет на Netlify (другой домен), а сервер — на Railway, так что
// браузер без явного разрешения CORS заблокирует запросы. CORS_ORIGIN —
// origin фронтенда в проде (или через запятую несколько, например
// прод-домен Netlify + его deploy-preview поддомен), задаётся в
// переменных окружения Railway. Если её нет (локальная разработка,
// process.env.CORS_ORIGIN не задан в .env) — cors({}) ведёт себя как
// прежний cors(): разрешает любой origin, ничего не меняется.
